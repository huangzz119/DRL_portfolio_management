...
def cnn_predictor(input_num, inputs, previous_action, scope):
    """
    the cnn predictor to give the corresponding weights for assets 
    """
    with tf.variable_scope(scope):
        ...
        out = tf.layers.dense(net, asset_dim, activation=tf.nn.softmax, kernel_initializer=w_init) 

    return out

def rnn_predictor(input_num, inputs, previous_action, scope):
    """
    the rnn predictor to give the corresponding weights for assets 
    """
    with tf.variable_scope(scope):
        ...
        net = tf.layers.flatten(net)
        net = tf.layers.dense(net, 256, activation=tf.nn.relu)  # [1, 6]
        
        w_init = tf.random_uniform_initializer(-0.01, 0.01)
...
class StockActor:
    """
    Input of the network is the state;
    ...
    def __init__(self,sess, asset_dim, window_size, feature_dim, learning_rate, tau, bench_size, nn = "cnn"):
        ...
        self.scopes=['rnn/online/actor','rnn/target/actor']
        ...
        # the action gradient will be provided by the critic network
        # then combine the gradients here
